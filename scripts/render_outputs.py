...
KIND_ICON = {"visit": "📍", "meal": "🥢", "transit": "🚕", "rest": "🛏️", "checkin": "🏨", "checkout": "🧳", "transport_major": "🚄", "free": "🌿", "buffer": "☕"}
DAY_ICON = ("🚄", "⛴️", "🏯", "🧳", "🌤️", "🗺️", "✨")
BUDGET_STATUS = {"known": "已确认", "estimate": "计划估算", "unknown": "待确认"}
...
def _html_item_card(c: Ctx, item: dict) -> str:
    ...
    return (
        ...
        f"<div class='item-body'><div class='item-topline'><span class='kind-badge'>{KIND_ICON.get(item.get('kind'), '•')} {esc(kind)}</span><span class='status-badge status-{esc(item.get('verification_status') or 'unknown')}'>{esc(verification)}</span><span class='booking-badge'>{esc(booking)}</span></div>"
...
def render_html(c: Ctx) -> str:
    ...
    for index, day in enumerate(it["days"], 1):
        ...
        days.append(f'<article class="day" id="day-{index}" data-day="{index}" data-day-date="{esc(day.get("date"))}"><div class="day-head"><span><b>D{index}</b><em>{DAY_ICON[(index - 1) % len(DAY_ICON)]}</em></span><div><h2>{esc(fmt_date(day.get("date")))} · {esc(day.get("theme"))}</h2><p>{esc(day.get("region") or "—")} · {esc(INTENSITY.get(day.get("intensity"), day.get("intensity")))}</p></div></div>{day_stats}{notes}{"".join(cards)}</article>')
    ...
    overview_rows = "".join(f"<tr><td><span class='overview-day'>{DAY_ICON[(n - 1) % len(DAY_ICON)]} D{n}</span></td><td>{esc(fmt_date(day.get('date')))}</td><td><b>{esc(day.get('theme'))}</b></td><td>{esc(day.get('region') or '待确认')}</td><td>{esc(INTENSITY.get(day.get('intensity'), day.get('intensity') or '待确认'))}</td><td>{esc('；'.join(day.get('notes') or []) or '按当天状态执行')}</td></tr>" for n, day in enumerate(it["days"], 1))
    route_rail = f"<div class='overview-table-wrap'><h3>每天去哪里</h3><div class='table-scroll'><table class='overview-table'><thead><tr><th>天</th><th>日期</th><th>主题</th><th>片区</th><th>强度</th><th>主要提醒</th></tr></thead><tbody>{overview_rows}</tbody></table></div></div>"
    ...
    variants = it.get("plan_variants", [])
    strategy_cards = "".join(
        f"<article class='route-option {'is-selected' if variant.get('selected') else ''}'><span class='section-kicker'>{'当前采用' if variant.get('selected') else '备选思路'}</span><h3>{esc(variant.get('title'))}</h3><p>{esc(variant.get('objective'))}</p><p class='muted'>取舍 · {esc('；'.join(variant.get('tradeoffs') or []) or '无')}</p></article>"
        for variant in variants
    )
    strategy_card = f"<article class='route-strategy'><div><span class='section-kicker'>路线为什么这样排</span><h3>当前选择与备选取舍</h3></div><div class='route-options'>{strategy_cards}</div></article>" if variants else ""
    assumptions = "<div class='reason-grid'>" + strategy_card + "<article><span class='section-kicker'>采用的假设</span><ul>" + "".join(f"<li>{esc(value)}</li>" for value in (it.get("assumptions") or ["暂无额外假设"])) + "</ul></article><article><span class='section-kicker'>重要取舍</span><ul>" + "".join(f"<li>{esc(value)}</li>" for value in (it.get("tradeoffs") or ["暂无额外取舍"])) + "</ul></article><article><span class='section-kicker'>暂未满足</span><ul>" + "".join(f"<li>{esc(value)}</li>" for value in (it.get("unmet") or ["当前没有列出的未满足项"])) + "</ul></article></div>"
    ...
    budget_rows = "".join(
        f"<tr><td><b>{esc(cat['name'])}</b></td><td>{esc(money(cat.get('paid') or 0, c.cur))}</td><td>{esc(money_range(cat.get('min'), cat.get('max'), c.cur))}</td><td><span class='budget-status'>{esc(BUDGET_STATUS.get(cat.get('status'), cat.get('status') or '待确认'))}</span></td><td>{esc(cat.get('note') or '—')}</td></tr>"
        for cat in it["budget"]["categories"]
    )
    budget_label = "预算可覆盖" if not unknown else "仍有待确认费用"
    remaining = max(0, limit - paid - high) if isinstance(limit, (int, float)) else None
    budget_body = f"<div class='budget-summary-grid'><article><small>已付</small><strong>{esc(money(paid, c.cur))}</strong><p>当前没有记录已支付项目</p></article><article><small>预计总成本</small><strong>{esc(money_range(paid + low, paid + high, c.cur))}</strong><p>包含交通、住宿、餐饮、门票与市内交通</p></article><article><small>全队预算上限</small><strong>{esc(money(limit, c.cur))}</strong><p>{esc(c.budget_brief())}</p></article><article><small>机动余量</small><strong>{esc(money(remaining, c.cur) if remaining is not None else '待确认')}</strong><p>{esc(budget_label)}</p></article></div><p class='budget-note'>{esc(it['budget'].get('note') or '价格为规划基准，完成预订后更新实际金额。')}</p><div class='table-scroll'><table class='budget-table'><thead><tr><th>类别</th><th>已付</th><th>计划金额</th><th>状态</th><th>口径说明</th></tr></thead><tbody>{budget_rows}</tbody></table></div>"
    ...
    lodging_cards = "".join(f"<article class='lodging-card'><div class='lodging-title'><span>🏨</span><div><small>候选住宿片区</small><h3>{esc(region.get('name') or '未定')}</h3></div></div><dl><div><dt>适合行程</dt><dd>{esc(region.get('suits_days') or '待确认')}</dd></div><div><dt>优势</dt><dd>{esc(region.get('pros') or '—')}</dd></div><div><dt>代价</dt><dd>{esc(region.get('cons') or '待确认')}</dd></div><div><dt>预算参考</dt><dd>{esc(region.get('budget_ref') or '预算待确认')}</dd></div></dl></article>" for region in it["lodging"].get("regions", []))
    lodging_body = f"<div class='strategy-card'><span class='section-kicker'>住宿策略</span><h3>一处连住，不换酒店</h3><p>{esc(it['lodging'].get('strategy') or '住宿策略未知')}</p></div><div class='lodging-grid'>{lodging_cards or '<p>住宿片区待确认。</p>'}</div>"
    transport_rows = "".join(f"<article class='transport-card'><div><span class='section-kicker'>🚄 {esc(major.get('kind') or '大交通')}</span><h3>{esc(major.get('from'))} <b>→</b> {esc(major.get('to'))}</h3><div class='transport-times'><span><small>出发</small><strong>{esc(fmt_dt(major.get('depart'), with_date=True))}</strong></span><span><small>到达</small><strong>{esc(fmt_dt(major.get('arrive'), with_date=True))}</strong></span></div></div><div class='transport-status'><span class='status-badge status-conditional'>{esc({'to_book':'待订','booked':'已订'}.get(major.get('status'), major.get('status') or '待确认'))}</span><strong>{esc(major.get('price_ref') or '价格待确认')}</strong></div><p class='transport-note'>{esc(major.get('note') or '—')}</p></article>" for major in it.get('transport_major', []))
    ...
    page_sections = "".join((
        _html_section("旅行总览", overview + route_rail, section_id="overview"), _html_section("现在要完成", action_body, section_id="actions"), _html_section("规划依据", constraint + assumptions, cls="section departure-hide", section_id="constraints"), _html_section("每天的安排", day_switcher + "".join(days), section_id="days"),
        _html_section("住宿说明", lodging_body, cls="section departure-hide", section_id="stay"), _html_section("大交通与市内移动", transport_body, cls="section departure-hide", section_id="transport"), _html_section("价格与预算", budget_body, cls="section departure-hide", section_id="budget"), _html_section("天气、预约与备用方案", weather_alternatives, cls="section departure-hide", section_id="alternatives"),
...
:root{{--ink:#174f4e;--deep:#103d3c;--coral:#c97564;--gold:#c78a2b;--paper:#fffdf8;--wash:#edf5f1;--cream:#f8f1e5;--line:#c9d9d2;--warm-line:#e5d7c0;--muted:#667772;--shadow:0 20px 60px #164d3d1c}}*{{box-sizing:border-box}}body{{margin:0;background:#e9efe9;color:var(--deep);font:15px/1.7 -apple-system,BlinkMacSystemFont,'PingFang SC','Noto Sans SC',sans-serif}}body:before{{content:'';position:fixed;inset:0;pointer-events:none;opacity:.18;background:repeating-linear-gradient(0deg,transparent 0 22px,#bad9ce 23px 24px)}}main{{max-width:1280px;margin:26px auto;padding:0 22px}}.page{{background:var(--paper);border:1px solid #d8e4dd;border-radius:28px;box-shadow:var(--shadow);overflow:visible}}header{{padding:58px 62px 42px;position:relative;background:linear-gradient(130deg,#fffdf7,#eff8f1);border-radius:28px 28px 0 0}}header:after{{content:'潮汐纸纹 · OFFLINE';position:absolute;right:42px;top:42px;color:#c5dcd2;letter-spacing:.18em;font-size:11px}}.eyebrow{{color:var(--coral);letter-spacing:.16em;font-weight:700}}h1{{font-family:Georgia,'Songti SC',serif;font-size:clamp(42px,6vw,82px);line-height:1.05;margin:16px 0;color:var(--ink);letter-spacing:-.04em}}h2{{font:700 30px/1.2 Georgia,'Songti SC',serif;color:var(--ink);margin:0 0 20px}}h3{{margin:0 0 6px;color:var(--ink)}}.muted,small,.meta{{color:var(--muted)}}.status{{display:inline-block;border:1px solid #d9b0a4;color:#a65e51;padding:4px 12px;border-radius:999px;background:#fff4ee}}button{{border:1px solid var(--line);background:#fff;padding:6px 10px;border-radius:999px;color:var(--ink)}}.facts,.columns{{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}}.facts>div,.columns>div,.action-grid article{{background:#fff;border:1px solid var(--line);border-radius:16px;padding:16px}}.action-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}}small{{display:block;font-size:12px;letter-spacing:.08em}}strong{{display:block;color:var(--ink);font-size:19px;margin-top:3px}}.content{{padding:6px 0 72px}}.section{{padding:30px;margin:22px 0;border:1px solid var(--line);border-radius:22px;background:#fff}}#overview{{background:#f1f7f4}}#actions{{background:#fff7ee;border-color:#ead0be}}#constraints{{background:#f8f4ec;border-color:var(--warm-line)}}#days{{background:#f7faf8}}#stay{{background:#edf6f3}}#transport{{background:#fff}}#budget{{background:#fff7e9;border-color:#ead8b7}}#alternatives{{background:#f5f3ed}}.day{{background:#fff;border:1px solid var(--line);border-radius:20px;padding:22px;margin:18px 0;box-shadow:0 8px 24px #174f4e0b}}.day-head{{display:flex;gap:16px;align-items:flex-start;border-bottom:1px dashed var(--line);padding-bottom:14px}}.day-head>span{{background:var(--ink);color:white;border-radius:16px;width:58px;min-width:58px;height:62px;display:grid;place-items:center;font-weight:700;line-height:1}}.day-head>span b{{font-size:15px}}.day-head>span em{{font-size:22px;font-style:normal}}.item{{display:grid;grid-template-columns:120px 1fr;gap:20px;padding:18px 0;border-bottom:1px solid #e5eee9}}.item:last-child{{border-bottom:0}}.time{{font-weight:700;color:var(--coral);font-variant-numeric:tabular-nums}}.time span{{color:#b5c8bf;padding:0 4px}}.item p{{margin:4px 0}}.meta{{font-size:13px}}.note{{color:var(--muted);margin:8px 0}}table{{width:100%;border-collapse:separate;border-spacing:0;margin-top:14px;background:#fff;border:1px solid var(--line);border-radius:15px;overflow:hidden}}th,td{{text-align:left;padding:12px;border-bottom:1px solid var(--line);vertical-align:top}}tr:last-child td{{border-bottom:0}}th{{color:var(--ink);background:var(--wash)}}.checklist{{display:grid;gap:10px}}.checklist label{{display:grid;grid-template-columns:28px 1fr;gap:8px;border:1px solid var(--line);border-radius:13px;padding:11px 14px;background:#fff}}.checklist label>span{{color:var(--coral);font-size:20px}}.checklist small{{grid-column:2}}.evidence-list p{{display:flex;justify-content:space-between;gap:16px;border-bottom:1px solid #e5eee9;padding:8px 0}}.evidence-list span{{color:var(--coral);white-space:nowrap}}details{{background:#fff;border:1px solid var(--line);border-radius:16px;padding:14px 16px}}summary{{cursor:pointer;font-weight:700;color:var(--ink)}}.source-list{{columns:2}}footer{{padding:24px 62px;background:var(--deep);color:#eaf5f0;font-size:13px;border-radius:0 0 28px 28px}}footer .muted{{color:#b8d4c9}}@media(prefers-color-scheme:dark){{body{{background:#112c2b}}.page{{filter:brightness(.92)}}}}@media(max-width:760px){{header,footer{{padding-left:22px;padding-right:22px}}.facts,.columns,.action-grid{{grid-template-columns:1fr 1fr}}.section{{padding:22px}}.item{{grid-template-columns:1fr;gap:4px}}.source-list{{columns:1}}}}@media(max-width:430px){{.facts,.columns,.action-grid{{grid-template-columns:1fr}}h1{{font-size:48px}}}}
...
def render_html(c: Ctx) -> str:
    ...
    nav_items = (("overview", "旅行总览"), ("actions", "现在要做"), ("constraints", "规划依据"), ("days", "每天的安排"),
                 ("stay", "住宿说明"), ("transport", "交通说明"), ("budget", "价格与预算"), ("alternatives", "天气与备用"), ("checklist", "出发清单"),
                 ("evidence", "核查依据"))
    ...
    route_day_nodes = "".join(f"<span class='hero-day'><b>{DAY_ICON[(index - 1) % len(DAY_ICON)]} D{index}</b><small>{esc(fmt_date(day.get('date')))}</small></span>" for index, day in enumerate(it.get("days", []), 1))
    hero_route = f'<div class="hero-route" aria-label="路线概览"><div class="hero-route-track"><span class="hero-endpoint"><b>{origin}</b><small>出发</small></span>{route_day_nodes}<span class="hero-endpoint"><b>{origin}</b><small>返程</small></span></div><p>{origin} → {destination} · {esc(c.date_range())} · {len(it.get("days", []))} 天</p></div>'
    ...
    css = """
.layout{display:grid;grid-template-columns:210px minmax(0,1fr);gap:26px;align-items:start;padding:26px 48px 72px}.sidebar{position:sticky;top:18px;align-self:start;height:fit-content;max-height:calc(100vh - 36px);overflow:auto;z-index:3;border:1px solid var(--warm-line);border-radius:18px;background:#fffaf1;padding:16px;box-shadow:0 8px 26px #46351c12}.sidebar-title{font-size:12px;letter-spacing:.14em;color:var(--coral);font-weight:700;margin-bottom:10px}.sidebar nav{display:grid;gap:3px}.sidebar a{color:var(--deep);text-decoration:none;padding:7px 10px;border-radius:10px}.sidebar a:hover,.sidebar a.active{background:var(--ink);color:#fff}.sidebar .side-note{border-top:1px solid var(--warm-line);margin-top:14px;padding-top:12px;color:var(--muted);font-size:12px}.section{scroll-margin-top:20px}.day-switcher{display:flex;gap:8px;position:sticky;top:12px;z-index:2;background:#f7faf8;padding:8px;border:1px solid var(--line);border-radius:999px;width:max-content;margin-bottom:18px}.day-switcher button[aria-pressed="true"]{background:var(--ink);color:#fff}.day.is-hidden{display:none}.progress-wrap{margin:4px 0 16px;color:var(--ink)}.progress-wrap>div:first-child{display:flex;justify-content:space-between;font-size:13px}.progress{height:7px;background:#dcebe3;border-radius:999px;overflow:hidden;margin-top:6px}.progress i{display:block;height:100%;width:0;background:var(--coral);transition:width .2s}.checklist input{accent-color:var(--coral)}.totop{position:fixed;right:24px;bottom:24px;opacity:0;pointer-events:none;transition:opacity .2s}.totop.visible{opacity:1;pointer-events:auto}body.departure .sidebar,body.departure .departure-hide,body.departure .day-switcher{display:none}body.departure .day{display:none}body.departure .day.departure-day{display:block;border-color:var(--coral);box-shadow:0 0 0 3px #c9756420}body.departure .layout{display:block;max-width:760px;margin:auto}.departure-note{display:none;color:var(--coral);font-weight:700}body.departure .departure-note{display:block}@media print{body:before,.sidebar,.day-switcher,.totop,#departure-mode{display:none!important}.layout{display:block;padding:0 40px}.page{box-shadow:none;border:0}}@media(max-width:760px){.layout{display:block;padding:18px 16px 50px}.sidebar{position:sticky;top:0;max-height:none;margin-bottom:18px;overflow-x:auto;overflow-y:hidden;padding:10px;background:#fffdf8f2;backdrop-filter:blur(8px)}.sidebar nav{display:flex;min-width:max-content}.sidebar .side-note,.sidebar-title{display:none}}
"""
    css += """
...
.place-row,.rule-row{display:flex;align-items:center;gap:8px;color:var(--deep);font-size:13px;margin-top:8px}.place-row span{color:var(--muted);font-size:12px}.rule-row{color:var(--muted)}.day-stats{display:flex;flex-wrap:wrap;gap:8px;margin:8px 0 14px}.day-stats span{border:1px solid var(--line);border-radius:999px;padding:3px 9px;color:var(--muted);font-size:12px;background:#fffdf8}
.hero-route{overflow-x:auto}.hero-route-track{position:relative;display:grid;grid-auto-flow:column;grid-auto-columns:minmax(94px,1fr);align-items:start;min-width:680px;padding:12px 0 4px}.hero-route-track:before{content:'';position:absolute;left:7%;right:7%;top:36px;height:4px;border-radius:999px;background:linear-gradient(90deg,var(--coral),var(--gold),var(--ink))}.hero-route-track>span{position:relative;z-index:1;display:grid;justify-items:center;text-align:center}.hero-route-track>span b{display:grid;place-items:center;min-width:64px;height:48px;padding:0 9px;border:3px solid var(--ink);border-radius:16px;background:var(--paper);color:var(--ink)}.hero-route-track>span small{margin-top:8px;letter-spacing:0;color:var(--muted)}.hero-route-track .hero-endpoint b{border-radius:999px;background:var(--ink);color:#fff}.hero-route>p{text-align:center;color:var(--muted);margin:8px 0 0}.overview-table-wrap{margin-top:28px}.overview-table{min-width:760px}.overview-day{display:inline-flex;align-items:center;gap:5px;white-space:nowrap;font-weight:800;color:var(--ink)}.lodging-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.lodging-card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:18px}.lodging-title{display:flex;gap:12px;align-items:center}.lodging-title>span{display:grid;place-items:center;width:44px;height:44px;border-radius:14px;background:var(--wash);font-size:24px}.lodging-card dl{margin:16px 0 0}.lodging-card dl>div{display:grid;grid-template-columns:82px 1fr;gap:10px;padding:8px 0;border-top:1px dashed var(--line)}.lodging-card dt{color:var(--muted)}.lodging-card dd{margin:0}.budget-summary-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.budget-summary-grid article{background:#fff;border:1px solid var(--warm-line);border-radius:16px;padding:16px}.budget-summary-grid article:nth-child(2),.budget-summary-grid article:nth-child(4){background:#fffaf0}.budget-summary-grid p{margin:6px 0 0;color:var(--muted);font-size:12px}.budget-summary-grid strong{color:#8b5a2f}.budget-note{padding:14px 16px;border-left:4px solid var(--gold);background:#fffaf0;border-radius:0 12px 12px 0}.budget-table{min-width:820px}.budget-status{display:inline-block;white-space:nowrap;border-radius:999px;background:#fff1d8;color:#8b5a2f;padding:3px 8px;font-size:12px}.transport-times{display:flex;gap:18px;margin-top:12px}.transport-times span{padding:8px 12px;border-radius:12px;background:var(--wash)}.transport-times strong{font-size:15px}.strategy-card h3{font-size:22px}.route-strategy{grid-column:1/-1;background:#eef6f3!important;border-left:5px solid var(--ink)!important}.route-options{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:12px}.route-option{border:1px solid var(--line)!important;background:#fff!important;padding:14px!important}.route-option.is-selected{border:2px solid var(--ink)!important;background:#f3faf6!important}@media(max-width:980px){.budget-summary-grid{grid-template-columns:repeat(2,1fr)}.lodging-grid,.route-options{grid-template-columns:1fr}}@media(max-width:520px){.budget-summary-grid{grid-template-columns:1fr}.section{padding:18px}.transport-times{display:grid;grid-template-columns:1fr 1fr}}
"""
    page = page.replace("</style>", css + "</style>", 1)
    script = """
<script>(function(){var links=[].slice.call(document.querySelectorAll('#section-nav a')),sections=links.map(function(a){return document.getElementById(a.dataset.section)}).filter(function(section){return section});function mark(id){links.forEach(function(a){a.classList.toggle('active',a.dataset.section===id)})}if(window.IntersectionObserver){var observer=new IntersectionObserver(function(entries){entries.forEach(function(entry){if(entry.isIntersecting)mark(entry.target.id)})},{rootMargin:'-18% 0px -70% 0px'});sections.forEach(function(section){observer.observe(section)})}mark('overview');var filters=[].slice.call(document.querySelectorAll('[data-day-filter]')),cards=[].slice.call(document.querySelectorAll('.day'));filters.forEach(function(button){button.addEventListener('click',function(){filters.forEach(function(b){b.setAttribute('aria-pressed',String(b===button))});cards.forEach(function(card){card.classList.toggle('is-hidden',button.dataset.dayFilter!=='all'&&card.dataset.day!==button.dataset.dayFilter)})})});var departure=document.getElementById('departure-mode');if(departure){departure.addEventListener('click',function(){var enabled=document.body.classList.toggle('departure');var today=new Date().toISOString().slice(0,10),active=cards.filter(function(card){return card.dataset.dayDate===today})[0]||cards[0];cards.forEach(function(card){card.classList.toggle('departure-day',card===active)});departure.textContent=enabled?'查看完整计划':'出发模式';if(enabled&&active)active.scrollIntoView({block:'start',behavior:'smooth'})})}document.querySelectorAll('.countdown').forEach(function(el){var d=new Date(el.dataset.deadline);if(!isNaN(d))el.textContent=' · 截止 '+d.toLocaleString()});var checks=[].slice.call(document.querySelectorAll('.checklist input')),progress=document.getElementById('check-progress'),bar=document.getElementById('check-progress-bar'),key='travel-plan-pro:{trip}:{version}';try{var saved=JSON.parse(localStorage.getItem(key)||'[]');checks.forEach(function(box,index){box.checked=saved[index]===true})}catch(e){}function update(){var done=checks.filter(function(box){return box.checked}).length,total=checks.length;if(progress)progress.textContent=done+'/'+total+' 已完成';if(bar)bar.style.width=(total?done/total*100:0)+'%';try{localStorage.setItem(key,JSON.stringify(checks.map(function(box){return box.checked})))}catch(e){}}checks.forEach(function(box){box.addEventListener('change',update)});update();var top=document.querySelector('.totop');window.addEventListener('scroll',function(){if(top)top.classList.toggle('visible',window.scrollY>500)});if(top)top.addEventListener('click',function(){window.scrollTo({top:0,behavior:'smooth'})})})()</script>
