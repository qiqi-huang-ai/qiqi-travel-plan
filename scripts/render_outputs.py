...
def render_html(c: Ctx) -> str:
    ...
    for index, day in enumerate(it["days"], 1):
        ...
        days.append(f'<article class="day" id="day-{index}" data-day="{index}"><div class="day-head"><span>D{index}</span><div><h2>{esc(fmt_date(day.get("date")))} · {esc(day.get("theme"))}</h2><p>{esc(day.get("region") or "—")} · {esc(INTENSITY.get(day.get("intensity"), day.get("intensity")))}</p></div></div>{notes}{"".join(cards)}</article>')
