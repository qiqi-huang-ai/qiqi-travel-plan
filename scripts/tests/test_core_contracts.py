...
class CoreContractTests(unittest.TestCase):
    ...
    def test_renderer_keeps_business_content_and_has_no_network_dependency(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotIn('class="floating-index"', html)
            self.assertIn('id="day-switcher"', html)
            self.assertIn("hero-route-track", html)
            self.assertIn("overview-table", html)
            self.assertIn('class="plan-notice"', html)
            self.assertIn("timeline-card", html)
            self.assertIn("reason-grid", html)
            self.assertIn("lodging-grid", html)
            self.assertIn("budget-summary-grid", html)
            self.assertIn("alternatives-table", html)
            self.assertNotIn("方案变体", html)
