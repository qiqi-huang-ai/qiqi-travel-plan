...
HTML 的信息架构分为“固定导航 + 主内容”：桌面端左侧是可点击的分区索引，移动端降级为横向索引；滚动后仍保留固定的“目录”浮动入口，避免用户为了切换分区反复回到页面顶部；每日行程提供“全部 / D1 / D2 …”筛选；滚动时当前分区会高亮。总览中用离线 CSS 旅程骨架表达天数，清单支持本地浏览器勾选并显示准备度，返回顶部和打印样式只改变阅读方式，不写回 `trip.json`。出发模式应只保留当天的行程和最紧急待办，而不是只改变视觉边框。这些是阅读层能力，业务数据仍以 `trip.json` 为唯一来源。
