...
面向旅行者的 HTML 默认先呈现“现在要完成”、每日行程、预算和清单；Provider 能力、历史决策、changelog、内部目标 ID 不得挤进主阅读流。事实、来源与待复查事项放在页面末尾的折叠“核查依据”中。不要为了字段覆盖率把内部记录追加为“补充记录”。

`verification_status: planned` 表示用户已经确认的行程基准，和 `verified` 的外部事实核查不同；它可以显示为“已确定计划”，但未购票、未订房仍必须保留对应的预约状态。清单的精确倒计时只读取可选的 `deadline_at`（ISO 8601）；`deadline` 保留给“尽快处理”等人类可读提示。
...
HTML 的信息架构分为“固定导航 + 主内容”：桌面端左侧分区索引在纵向滚动时保持固定，移动端降级为顶部横向索引；每日行程提供“全部 / D1 / D2 …”筛选；滚动时当前分区会高亮。总览用连续路线表达天数，并紧接“每天去哪里”摘要；住宿、交通、价格分别作为一级板块，不把预算混入住宿说明。清单支持本地浏览器勾选并显示准备度，返回顶部和打印样式只改变阅读方式，不写回 `trip.json`。出发模式应只保留当天的行程和最紧急待办，而不是只改变视觉边框。这些是阅读层能力，业务数据仍以 `trip.json` 为唯一来源。
